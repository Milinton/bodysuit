...
`.trim(), m.message, {
  ...
  async onCall(json) {
    ...
    await this.sendMessage(from, '*_⚠️NO LLAMAR AL BOT POR FAVOR, NO QUEREMOS BLOQUEARL@, EVITENOS LA PENA DE HACERLO⚠️_*\n\n*_⚠️SI ESTE INCIDENTE SE REPITE USTED SERA BLOQUEAD@⚠️_*', MessageType.extendedText)
  }
}

global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '*_Este comando solo puede ser utilizado por el creador del BoT_*',
    owner: '*_Este comando solo puede ser utilizado por el creador del Bot y subbots_*',
    mods: '*_Este comando solo puede ser utilizado por moderadores_*',
    premium: '*_Este pedido es solo para miembros premium_*\n\n*_Nota: La opcion de usuarios premium esta siendo desactivada, si este mensaje se te fue enviado reportalo con los comandos del area de reportes_*',
    group: '*_Este comando solo se puede usar en grupos!_*',
    private: '*_Este comando solo se puede usar en el chat privado!_*',
    admin: '*_Este comando es solo para los Admins del grupo!_*',
    botAdmin: '*_Ascender el bot como Admin para usar este comando!_*',
    unreg: '*_Regístrese para utilizar esta función escribiendo:_*\n\n*_#daftar nombre.edad_*\n\n*_Ejemplo:_*\n*_#daftar Shadow.18_*\n\n*_Este registro se solicita con el fin de que el Bot no se sature_*\n\nNota: No escriba los "*" ni los "_"'
  }[type]
